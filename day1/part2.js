...
const solution = (input) => {
  ...
  const replaceWordsWithNumbers = (str) => {
    const replacements = [];
    const arrayOfNumbersWords = Object.keys(numbersMap);
    arrayOfNumbersWords.forEach((word) => {
      let indexOfWord = 0;
      while (str.indexOf(word, indexOfWord) !== -1) {
        indexOfWord = str.indexOf(word, indexOfWord);
        replacements.push({
          index: indexOfWord,
          number: numbersMap[word],
        });
    ...
    replacements.forEach((replacement) => {
      str = str.split("");
      str[replacement.index] = replacement.number;
      str = str.join("");
    });
    return str;
  };
  ...
  const getNumberValue = (str) => {
    str = replaceWordsWithNumbers(str);

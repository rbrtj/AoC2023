...
const solution = (input) => {
  ...
  arr.forEach((str) => {
    ...
    const tempNumbersLength = tempNumbers.length;
    if (!tempNumbersLength) {
      return;
    }
    if (tempNumbersLength < 2) {
      const res = tempNumbers[0] + tempNumbers[0];
      numbers.push(parseInt(res));
    }
    if (tempNumbersLength >= 2) {
      const res = tempNumbers[0] + tempNumbers[tempNumbers.length - 1];
      numbers.push(parseInt(res));
    }
  });
  const sol = numbers.reduce((acc, cur) => acc + cur, 0);
  console.log(sol);
  return sol;
};

...
const solution = (input) => {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    games[i] = game.substring(indexOfColon + 2, game.length);
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    for (let round of game) {
      ...
      for (let color of Object.keys(bag)) {
        ...
        if (gameMap[color] > gemsCount) {
          continue;
        }
        gameMap[color] = gemsCount;
      }
    }
    gamesMaxGems.push(gameMap);
  }
  const powers = gamesMaxGems.map((game) => game.red * game.blue * game.green);
  console.log(powers);
